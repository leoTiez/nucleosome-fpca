...
for i =1:9
    ...
    %
    E = evalclusters(AlignedProfile_g,'kmeans','silhouette','klist',1:10);
    Fig1=figure(1);plot(E);
    %
    rng('default');
    
    [cidx, ctrs] = kmeans(AlignedProfile_g,E.OptimalK,'dist','corr','rep',20,'disp','final');
    Fig2=figure(2);
    for c = 1:E.OptimalK
        subplot(E.OptimalK,1,c);
        plot(AlignedProfile_g((cidx == c),:)');
        axis tight
        xlabel('x (bp)')
