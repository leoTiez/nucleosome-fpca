...
def run(data_profiles, single_pred=True, save_fig=False, save_prefix=''):
    ...
    for data_name, dp in data_profiles.items():
        ...
        plt.bar(np.arange(len(names)) + .3, 1. - percentage, 0.4, bottom=0, color='tab:orange', label='Correct')
        plt.bar(np.arange(len(names)) + .3, percentage, 0.4, bottom=1. - percentage, color='tab:blue', label='Wrong')
        plt.ylim((.4, 1.1))
        plt.plot(np.arange(len(names) + 1), .5 * np.ones(len(names) + 1), linestyle='--', color='black', lw=2)
